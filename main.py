...
def train(dataset: DataDir, model : NeuralNetwork, loss_fn : nn.MSELoss, optimizer : torch.optim.Adam, device : str, train_set : list):
...
def test(dataset : DataDir, model : NeuralNetwork, loss_fn : nn.MSELoss, device : str, test_set : list):
...
def main() -> None:
    ...
    # set training choice
    choice = [0, 2, 5, 7, 9]  # indexes in the sorted concentration list
    ...
    for t in tqdm(range(epochs)):
        if DEBUG_MODE:
            print(f"Epoch {t+1}\n -----------------------")
        avg_loss = train(new_dataset, model, loss_fn, optimizer, device, choice)
        losses.append(avg_loss)
        val_loss = test(new_dataset, model, loss_fn, device, for_test)
